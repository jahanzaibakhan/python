#!/usr/bin/env python3
"""
WordPress/WooCommerce Health Monitor (cloud2.py)

Same as cloud1.py, but saves the investigation report as a plain-text `.txt` file
instead of `.log` so it can be opened in a browser on Cloudways/nginx stacks that
block `*.log` URLs (403). Use the HTTPS URL in SiteSleuth's investigation log field.
...
class HealthReportGenerator:
    ...
    def _save_json_report(self):
        ...
        import sys
        filename = f"wp_health_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
